...
@Service
public class ReductionTsCarInfoNewServiceImpl implements ReductionTsCarInfoNewService{
...
//				LOG.debug("["+carNo+"] ReductionTsCarInfo responseXml Decrypt [" + responseXml + "]");
			...
			LOG.info(" reductionTsCarInfoResponseBody  cbdLt : " + reductionTsCarInfoResponseBody.getCbdLt());
			LOG.info(" reductionTsCarInfoResponseBody  vhctySeNm : " + reductionTsCarInfoResponseBody.getVhctySeNm());
			...
			if("00".equals(reductionTsCarInfoResponseBody.getProcessImprtyResnCode())) {
				...
				// 차종이 화물이면서 대형인 경우
								...
								( 
									"승합".equals(reductionTsCarInfoResponseBody.getVhctyAsortNm())
									|| "화물".equals(reductionTsCarInfoResponseBody.getVhctyAsortNm())
									|| "특수".equals(reductionTsCarInfoResponseBody.getVhctyAsortNm())
									)
								&& (
									"중형".equals(reductionTsCarInfoResponseBody.getVhctySeNm()) 
									|| "대형".equals(reductionTsCarInfoResponseBody.getVhctySeNm()) 
								)
							){
					LOG.info(" reductionTsCarInfo BigCar True");
					adminInfomationParkingCar.setReductionYn("Y");
					adminInfomationParkingCar.setBigCarYn("Y");
				}
				else {
	...
	private ReductionTsCarInfoResponseBody parsingResultFromResponseXml(String responseXml, String airportCode, String parkingManageNumber, String carNo) throws Exception{
		...
		try {
			...
			if("00".equals(reductionTsCarInfoResponseBody.getProcessImprtyResnCode())){
				if (responseXml.contains("<cbdLt/>")) {
					reductionTsCarInfoResponseBody.setCbdLt("");
				} else {
					reductionTsCarInfoResponseBody.setCbdLt(responseXml.split("<cbdLt>")[1].split("</cbdLt>")[0]);
				}
				if (responseXml.contains("<vhctySeNm/>")) {
					reductionTsCarInfoResponseBody.setVhctySeNm("");
				} else {
					reductionTsCarInfoResponseBody.setVhctySeNm(responseXml.split("<vhctySeNm>")[1].split("</vhctySeNm>")[0]);
				}				
			}
